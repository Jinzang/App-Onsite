...
package App::Onsite::Support::ConfiguredObject;
...
sub new {
	...
	my $self = App::Onsite::Support::ConfiguredObject->create_object();
	...
	$obj = $obj->populate_object(\%configuration)
		   if $obj->isa('App::Onsite::Support::ConfiguredObject');
...
sub parameters {
	my ($self) = @_;
	return (cf => {DEFAULT => 'App::Onsite::Support::ConfigFile'});
}
...
sub load_object {
	...
	if ($class->isa('App::Onsite::Support::ConfiguredObject')) {
	...
	foreach my $field (keys %$self) {
		...
		if (exists $self->{$field}{DEFAULT}) {
			...
			$self->{$field} = $subobject->populate_object($configuration)
				if $subobject->isa('App::Onsite::Support::ConfiguredObject');
		}
	}
...
App::Onsite::Support::ConfiguredObject is the base class for configured objects

=head1 SYNOPSIS

	use App::Onsite::Support::ConfiguredObject;
